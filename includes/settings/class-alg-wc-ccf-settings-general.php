...
class Alg_WC_CCF_Settings_General extends Alg_WC_CCF_Settings_Section {
	...
	function get_settings() {
		$settings = array(
			...
			array(
				'desc'    => (
					__( 'Each field', 'custom-checkout-fields-for-woocommerce' ) . '. ' .
					$this->placeholders_desc( array( '%label%', '%value%' ) )
				),
			...
			array(
				...
				'desc'                => sprintf(
					...
					__( 'Will be used in all fields as the middle part of ID, wrapped in the field section, and the field number, e.g., %s.', 'custom-checkout-fields-for-woocommerce' ),
					'<code>_billing_<span style="color:black;font-weight:bold;">' .
						ALG_WC_CCF_KEY .
					'</span>_1</code>'
				),
				'type'                => 'text',
