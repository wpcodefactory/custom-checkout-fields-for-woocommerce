...
class Alg_WC_CCF_Order_Details {
	...
	function __construct() {
		...
		add_action(
			...
			array( $this, 'add_custom_shipping_fields_to_admin_order_display' ),
			PHP_INT_MAX
		);
		add_action(
		...
		add_action(
			...
			array( $this, 'add_custom_fields_to_emails' ),
			PHP_INT_MAX,
			2
		);
		if ( 'yes' === alg_wc_ccf_get_option( 'add_to_order_received', 'yes' ) ) {
			add_action(
				...
				array( $this, 'add_custom_fields_to_view_order_and_thankyou_pages' ),
				PHP_INT_MAX
			);
		}
	}
	...
	function get_order_id( $_order ) {
		...
		if ( ! isset( $this->is_wc_version_below_3_0_0 ) ) {
			$this->is_wc_version_below_3_0_0 = version_compare(
				get_option( 'woocommerce_version', null ),
				'3.0.0',
				'<'
			);
		}
		...
		foreach ( $fields_data as $field_data ) {
			...
			if (
				...
				$replaced_values = array(
					...
					'%value%' => (
						is_array( $field_data['_value'] ) ?
						implode( ', ', $field_data['_value'] ) :
						$field_data['_value']
					),
				);
	...
	function add_woocommerce_admin_fields( $fields, $section ) {
		...
		foreach ( $fields_data as $field_data ) {
			...
			$key = (
				$field_data['_key'] . '_' .
				$field_data['_field_nr'] .
				(
					! empty( $field_data['_key_suffix'] ) ?
					'_' . $field_data['_key_suffix'] :
					''
				)
			);
			...
			$fields[ $key ] = array(
				'type'          => $type,
				'label'         => apply_filters(
					'alg_wc_ccf_admin_order_field_label',
					(
